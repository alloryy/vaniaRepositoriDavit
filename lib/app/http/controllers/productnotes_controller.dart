import 'package:project_1/app/models/productnotes_model.dart';
...
class ProductnotesController extends Controller {
    ...
    Future<Response> index() async {
     ...
     Future<Response> store(Request request) async {
        ...
        final existingProductnotes = await ProductnotesModel().query().where("note_id","=", requestData['note_id']).first();
        ...
        await ProductnotesModel().query().insert(requestData);
     ...
     Future<Response> show() async {
        try {
        final listProductnotes = await ProductnotesModel().query().get();
        return Response.json({
     ...
     Future<Response> update(Request request,int note_id) async {
          try {
            ...
            final productnotes = await ProductnotesModel().query().where('note_id','=',note_id).first();
            ...
            await ProductnotesModel().query().where('note_id','=',note_id).update(listProductnotes);
     ...
     Future<Response> destroy(int note_id) async {
        ...
        await ProductnotesModel().query().where("note_id",'=',note_id).delete();
